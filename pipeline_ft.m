function [cfg, data] = pipeline_ft(experiment, participant)
% 1. read the data
...
try
    %ICA decomposition
    data_epoch_1s.comp               = ft_componentanalysis(cfg_epoch_1s, data_epoch_1s);
    
    %Show elapsed time
    preproc_time            = toc;
    disp(['ICA time: '  num2str(preproc_time) ' seconds']);
catch
...
%% Helper functions
    ...
    function [cfg_lex, data_lex, cfg_gram, data_gram] = epoch_data(condition)
        cfg.trialdef.eventtype = 'STATUS';
        if strcmp(condition, 'gram')
